...
const Switch = React.forwardRef(({ className, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      "peer inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent shadow-sm transition-colors focus:visible-none focus:visible-2 focus:visible-ring focus:visible-offset-2 focus:visible-offset-background disabled-not-allowed disabled:opacity-50 data-[state=checked]-primary data-[state=unchecked]-input",
      className,
    )}
    {...props}
    ref={ref}
